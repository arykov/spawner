...
class function TRegExpr.InvertCaseFunction (const Ch : REChar) : REChar;
  ...
  { if we're using windows, we'll just use the Windows.pas CharUpper and
    CharLower functions. Otherwise, we'll use our own functions }
  {$IFDEF MSWINDOWS}
  {$ELSE}
  function UpperChar (lpsz : Char) : Char;
  begin
    UpperChar := UpCase(lpsz);
  end;

  function LowerChar (lpsz : Char) : Char;
  begin
    case ch of
      'A'..'Z' : LowerChar := CHR (ORD(ch)+31);
    else
      LowerChar := ch;
    end;
  end;
 ...
 begin
  ...
  {$ELSE}
    Result := {$IFDEF FPC}AnsiUpperCase (Ch) [1]{$ELSE} REChar (UpperChar (Ch)){$ENDIF};
    if Result = Ch
     then Result := {$IFDEF FPC}AnsiLowerCase (Ch) [1]{$ELSE} REChar (LowerChar (Ch)){$ENDIF};
  {$ENDIF}
   end;
